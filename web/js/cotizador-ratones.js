(function () {
  ...
  /* ============================================================
     Cotizador especializado de Roedores — clon de js/cotizador-termitas.js,
     mismo patron: 1 archivo por servicio para que no se pisen entre si.
     ============================================================ */
  ...
  var PAGE_SLUG = 'landing-ratones';

  /* ── Labels para WhatsApp / Sheet ── */
  var LBL = {
    indicio:   { 'ruidos':'ruidos en el entretecho', 'cables':'cables roídos', 'excrementos':'excrementos cerca de alimentos', 'galerias':'orificios o galerías' },
  ...
  var STEPS = [
    {
      ...
      options: [
        { value:'ruidos',      label:'Ruidos de noche',   icon:'🔊' },
        { value:'cables',      label:'Cables roídos',     icon:'⚡' },
        { value:'excrementos', label:'Excrementos',       icon:'⚠️' },
        { value:'galerias',    label:'Orificios/galerías', icon:'🕳️' }
      ]
    },
    {
  ...
  function buildWARatones(ans, contact, ref) {
    var indicio = LBL.indicio[ans.indicio] || ans.indicio;
    ...
    var msg     = intro + 'Necesito una visita técnica gratuita para ratones ' + loc + '(vi ' + indicio + '). ¿Cuándo me pueden visitar?';
  ...
  function createCotizador(widgetEl) {
    ...
    /* ── Step: resultado — siempre "visita tecnica" (mismo criterio que
       termitas: no hay tarifa plana de roedores, depende de m² y accesos) ── */
    function renderResult() {
      var ans    = state.answers;
      var nombre = state.contact.nombre ? state.contact.nombre.split(' ')[0] : '';
      var waRatones = buildWARatones(ans, state.contact, state.refCode);
      ...
      div.innerHTML =
        '<div class="cot-result-header">' +
          ...
          '<p class="cot-result-sub">El control de <strong>roedores</strong> requiere una inspección en terreno para darte un presupuesto exacto.</p>' +
        '</div>' +
        '<div class="cot-result-cards">' +
          ...
          '<div class="cot-result-card">' +
            ...
            '<span class="cot-card-value cot-card-value--sm">Identificación de especie, puntos de acceso y número de estaciones necesarias</span>' +
          '</div>' +
        '</div>' +
        '<div class="cot-result-disclaimer">' +
          '<strong>¿Por qué no hay precio online?</strong> El control de roedores se cotiza por número de estaciones de cebado y accesos a sellar. Sin ver tu propiedad no podemos darte un valor justo.' +
        '</div>' +
        '<p class="cot-result-human">Serás atendido por el representante técnico de A&C Soluciones.</p>' +
        '<div class="cot-result-ctas">' +
          '<a href="' + waRatones + '" target="_blank" rel="noopener noreferrer" class="btn btn-secondary cot-wa-btn">' +
