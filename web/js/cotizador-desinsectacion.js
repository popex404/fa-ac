(function () {
  ...
  /* ============================================================
     Cotizador especializado de Desinsectación (arañas/avispas/hormigas)
     — clon de js/cotizador-termitas.js, mismo patron.
     ============================================================ */
  ...
  var PAGE_SLUG = 'landing-desinsectacion';

  var LBL = {
    indicio:   { 'aranas':'arañas', 'avispas':'avispas', 'hormigas':'hormigas', 'no-seguro':'no está seguro de la especie' },
  ...
  var STEPS = [
    {
      id: 'indicio', question: '¿Qué plaga tienes?', subtitle: 'Elige la que más se parece a lo que viste',
      options: [
  ...
  function datosBloque(ans, contact, ref) {
    ...
    return '\n\n--- DATOS DE COTIZACIÓN (DESINSECTACIÓN) ---' +
           '\nRef: ' + ref +
  ...
  function buildWADesinsectacion(ans, contact, ref) {
    var indicio = LBL.indicio[ans.indicio] || ans.indicio;
    ...
    var msg     = intro + 'Necesito una visita técnica gratuita, tengo problema de ' + indicio + ' ' + loc + '. ¿Cuándo me pueden visitar?';
  ...
  function postWebhook(ans, contact, cta, ref) {
    ...
    var payload = {
      ...
      plaga:      ans.indicio || 'desinsectacion',
  ...
  function createCotizador(widgetEl) {
    ...
    function renderResult() {
      ...
      var wa = buildWADesinsectacion(ans, state.contact, state.refCode);
      var plagaLbl = LBL.indicio[ans.indicio] || 'la plaga';
      ...
      div.innerHTML =
        '<div class="cot-result-header">' +
          ...
          '<p class="cot-result-sub">El tratamiento de <strong>' + plagaLbl + '</strong> requiere una inspección en terreno para darte un presupuesto exacto.</p>' +
        '</div>' +
        '<div class="cot-result-cards">' +
          ...
          '<div class="cot-result-card">' +
            ...
            '<span class="cot-card-value cot-card-value--sm">Identificación de la especie, extensión del problema y método de tratamiento</span>' +
          '</div>' +
        '</div>' +
        '<div class="cot-result-disclaimer">' +
          '<strong>¿Por qué no hay precio online?</strong> El tratamiento se cotiza según la extensión del área afectada. Sin ver tu propiedad no podemos darte un valor justo.' +
        '</div>' +
